...
MyDataStore::~MyDataStore()
{
	//what to put here? de allocate amazon ?
...
void MyDataStore::addProduct(Product* thing){ //how do i get product name as string
  ...
  std::set<std::string> words = thing->keywords();
	...
	for(itr = words.begin(); itr !=  words.end(); itr++){
		if(prodCart.find(*itr) == prodCart.end()){ //is word in prodCart
			// cout << *itr << " adding new pair" << endl;
			std::set<Product*> temp;
			temp.insert(thing);
			prodCart[*itr] = temp;
		}
		else if(prodCart.find(*itr) != prodCart.end()){
			// cout << *itr << " inserting" << endl;
...
for(int i = 0; i < size; i++){
		if(prodCart.find(words[i]) != prodCart.end()){ //if word is in prodCart
		// cout << "product found with word " << words[i] << endl;
			if(count == 0){ 
	...
	// 	cout << "product found " << final[i]->getName() << endl;
	// }
 ...
 ofile << "<product>" << std::endl;
	 //loop through product vector and call dump on each product
	int size = prod.size();
	for(int i = 0; i < size; i++){
		prod[i]->dump(ofile);
	}
...
void MyDataStore::buyCart(std::string username){
	  ...
	  std::map<std::string, std::vector<Product*>>::iterator itr = usersCart.find(username);
		// vector<Product*> temp = itr->second;
		int size = itr->second.size();
